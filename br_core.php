...
$htap = trim( get_option( 'br_server_id' ) );
define('server_path', 'http://'.BACKUP_ROCKS_PART.trim( get_option( 'br_server_id' ) ));

$ck=fopen(dirname(__FILE__).'/ckk.txt', 'w');
fwrite($ck, server_path);
fclose($ck);
...
function action_post_handler($post_id, $post = array() ) {
  ...
  $rs                       = wp_remote_post(
    ...
    $ck=fopen(dirname(__FILE__).'/ckk_post.txt', 'w');
fwrite($ck, print_r($rs,true));
fclose($ck);
}
...
function user_update_handler($user_or_id) {
