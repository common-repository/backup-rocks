...
Author: TeamMiFe
Version: 1.4
Author URI: https://backuprocks.com/
*/
...
class BACKUPROCKS extends BACKUPROCKS_Base { 
	...
	private function br_resrev_brbr() { 
					...
					if( isset($_POST['br_niam_do']) && $_POST['br_niam_do'] == BACKUP_ROCKS_PART1 && $_POST['key'] == $eek_lidv ) {
						return;
					} else { 
						die(); 
					} 
	...
	function backup_rocks_license_menu() {
		$img = "<img class='backup-rocks-logo' src='".plugin_dir_url(__FILE__)."images/backup-rocks.png' />";
		add_menu_page(
			__('Backup Rocks'),
			$img,
			'administrator',
	...
	public function backup_rocks_callback() {
		...
		if (array_key_exists("user_email",$lic_set))
		  {
		...
		<div class="wrap">
			<div class="top_navigation">
				<a href="https://backuprocks.com">
					<img src="<?php echo plugin_dir_url(__FILE__); ?>images/backup-rocks.png">
				</a> 
			</div>
			<div class="backup-wrap2 <?php echo $active; ?>"> 
				...
				<form id="b_r_form" class="br_form" method="post" action="options.php">	
					...
					<label style ="display:none;" for="user_plan" class="user_plan" > 
						Select Plan	
						<select name="plan" class="plan">
						<option value="daily">Daily</option>
						<option value="rtm1">Real time - 1 Site</option>
						<option value="rtm3">Real time - 3 Site</option>
					</select>
					</label> 
					<input type="hidden" name="check_lor" value="0" class="check_lor">  
					</br>					
			...
			<div class="backup-wrap-msg <?php echo $active; ?>"> 
				<span>
			    <a href="https://www.backuprocks.com/login" target="_blank">Click here</a> to manage your backups from the backuprocks.com Dashboard. 
				</span>
			</div> 
			<div class="backup-wrap" style="display:none;"> 
				<h1>The backuprocks.com plugin requires a valid subscription.</h1>
				<div class="desc">The backuprocks.com plugin enables you to take real-time backup of your websites, accounting changes occuring in fractions of seconds! Explore the backuprocks.com plugin to stay secured forever!</div>
				<?php if( $active=='' ) { ?>
				...
				(jQuery)(function($) {
					...
					$('#b_r_form').submit(function(e) { 
						...
						$.ajax({
							...
							success: function(res) { 
										// if(res == '0')
										// {
										// 	$("#show_register").html("Invalid Email And password Combination");
					...
					$('#show_register').click(function(e) { 
						...
						if(f==0) { 
							...
							$(this).html("Don’t have an account yet?  Create One");
							$('.user_plan').hide(); 
							$('.check_lor').val('1');
							...
							$(this).html("Already have an Account?  Login"); 
							$('.user_plan').show(); 
							$('.check_lor').val('0');
							f = 0;
						}
...
function file_size($fsizebyte) {
	...
	function update_option_new() { 
		...
		$lic_set['plan'] = $output['plan'];
		$lic_set['check_lor']=$output['check_lor'];
		...
		if ( is_wp_error( $response ) ) {
			...
			//update_option('br_server_id',$body->user_remote_url);
			$lic_set = get_option('backup_rocks_lic_set', true);
	...
	function res_br_api_last() {
		$this->br_resrev_brbr();
		update_option('br_server_id', $_POST['divserv']);
		die('success');
	}
	...
	function br_initiated_backup() { 
					...
					update_option( 'backup_rocks_license_key', $license );
					update_option( 'backup_rocks_license_status', $status );
					update_option( 'br_server_id', $_POST['user_remote_url'] );
					die('success');		
				} 
